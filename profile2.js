...
async function checkApplicationStatus(donorEmail) {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/application-status/${donorEmail}/${requesterEmail}`);
...
async function displayApplicants(donorEmail) {
    try {
        const response = await fetch(`http://localhost:3000/api/applications/${encodeURIComponent(donorEmail)}`);
